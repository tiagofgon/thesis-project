...
class NPool
   ...
   std::unique_ptr<ThDeque<TaskGroup*>> QJob;
   //std::unique_ptr< std::unique_ptr<TaskGroup> > QJob;
   //ThDeque<Task*>          *QTask;   // reference to internal task queue
   std::unique_ptr<ThDeque<Task*>> QTask; //
   //std::unique_ptr<  ThDeque< std::unique_ptr<Task> >  >QTask;
   std::map<int, ThDeque<Task*>* > MTask; // Cada thread tem uma queue de tasks
