...
// ******************************************
 * ------------------------------------------
 * This is source file TwoGauss.C
 * Running two asynchronous correlated Gaussian generators 
 * =======================================================*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
int main(int argc, char **argv)
    ...
    for(sample=1; sample<=nS; sample++)
        ...
        GV2->Request_Vector(nSteps);  // send request to GV2
        // ------------------------
	for(i=0; i<size; i++)        //: meanwhile, treat GV1 data
           for(j=0; j<size; j++) 
	       {
	       scr = M1[i]*M1[j];
	       MV[i][j] += scr;
        ...
        // -------------------------
        GV1->Request_Vector(nSteps);   // send request to GV1
        GV2->Wait_For_Request();
        // -------------------------
	for(i=0; i<size; i++)        // meanwhile, treat GV2 data
           for(j=0; j<size; j++) 
	       {
	       scr = M2[i]*M2[j];
	       MV[i][j] += scr;
        ...
        // ----------------------------
    	if( sample%50 == 0 ) printf("\n %d samples done", 2*sample);
	}	// end of samples loop

     GV1->Wait_For_Request();   // wait for GV1 to finish
     T.Stop();
     ...
     printf("\n\n Timing for two generators with %d threads each", nTh/2);
     T.Report();
     Close_Job();
     }
...
void Init_Job()
    ...
    Input_Data();
    Alloc_Vectors();
    GV1 = new GaussVec(D, M1, 3*N, nTh/2, delta, pb, 999);
    GV2 = new GaussVec(D, M2, 3*N, nTh/2, delta, pb, 777);
    }
