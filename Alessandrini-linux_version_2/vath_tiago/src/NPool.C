...
NPool::NPool(int nTh, int nMax)
   ...
   // QTask1 = new ThDeque<Task*>();
   // QTask2 = new ThDeque<Task*>();
...
void NPool::FlushTasks(TaskGroup *tg)
   ...
   for(pos=tg->LT.begin(), i=0; pos!=tg->LT.end(); pos++, i++)
      ...
      //Task *aux = *pos;
      // cout << "adicionei à thread " << (i%nThreads)+1 << endl;
      // AQ[(i%nThreads)+1]->Add(t);
...
void NPool::TPool_Thread(int n)
   ...
   bool state;
   Task* T, *my_parent;
   //std::thread::id rank1 = std::this_thread::get_id();
   ...
   for(;;) // Here we start an infinite loop
      ...
      // cout << "aqui" << endl;
      // T = (AQ[rank]->Remove(state));
      // cout << "rank " << rank << endl;
      // if((rank%2) == 0)
      //    T = QTask1->Remove(state);
      // else
      //    T = QTask2->Remove(state);
...
int NPool::SuspendAndRunTask()
   ...
   // if((rank%2) == 0)
   //    T = QTask1->TryRemoveBack(state);
   // else
   //    T = QTask2->TryRemoveBack(state);

   T = QTaskArray[rank]->TryRemoveBack(state);

   //T = (AQ[rank]->TryRemoveBack(state));
