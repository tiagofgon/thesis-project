...
//
// Uses the ThPeer thread pool facility.
//
...
int main (int argc, char *argv[])
  ...
  // Get the sum limits and the number of threads from
   ...
   B = atof(argv[2]);
   nTh = atoi(argv[3]);
   printf("\n Testing Blocking Barrier class \n");
   ...
   for(int n=0; n<16; n++) rank[n] = n;
   
   for(int n=1; n<=nTh; n++)
      pthread_create(&threads[n], NULL, thread_fct, &rank[n]);
