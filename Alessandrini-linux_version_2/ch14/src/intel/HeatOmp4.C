...
void TaskFunction()
   ...
   // -------------------------------------------------------------
   // First, fix the thread strip range.
   // We know that he horizontal size N is exactly divisible by
   // the number of threads. This makes very easy the determination
   // of the current thread strip range
   // -------------------------------------------------------------
   int rsize = N/nThreads;
   rank = omp_get_thread_num();     // in [0, nThreads-1]
   beg = rank * rsize;
   end = (rank+1) * rsize;
   if(rank==0) beg++;              // correct for left boundary
   if(rank==(nThreads-1)) end--;   // correct fr tight boundary
