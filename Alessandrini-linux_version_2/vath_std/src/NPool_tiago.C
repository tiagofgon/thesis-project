...
NPool::NPool(int nTh, int nMax)
   ...
   // Create task queue for each thread, tiago
   // --------------------------
   for(int i=1; i<=nTh; i++){
      auto *QTask_aux = new ThDeque<Task*>();
      MTask.insert(make_pair(i, QTask_aux));
   }
...
NPool::~NPool() 
    ...
    //delete map of queues, tiago
      for(auto & [key, val] : MTask){
         delete val;
      }
    }
...
int NPool::SubmitJob(Task *T)
   {
   bool flushflag;
   int rank = TM->GetRank(); // TM = threadmanager
...
int NPool::SubmitJob(TaskGroup *TG)
   {
   cout << "SubmitJob foi chamado" << endl;
   bool flushflag;
   int rank = TM->GetRank(); 
   if(rank>0) return 0;   // client threads have rank (-1)
   else
...
void NPool::TPool_Thread()
   ...
   rank = TM->GetRank();                         // saber o rank desta worker thread
   
   for(;;) // Here we start an infinite loop
      {
      T = QTask->Remove(state);      // read task address 
      //T = MTask[rank]->Remove(state);
     ...
     // Get the JobMgr in charge of this task
      ...
      // -------------------------------
          ...
          if(nWorkers==0) FlushJobQueue(); // executa um novo job
          }
      }  // end for           
   }
