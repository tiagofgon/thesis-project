...
// other initial heat configurations.. 
// ------------------------------------------------------------
...
// Global data
// ***********
OBLock BL;                   // pipeline with boolean lock
...
void FirstStage()
   ...
   for(count=1; count<steps; count++)
      ...
      BL.Wait_Until(false);    // synchronize with next stage
      BL.SetState(true);
      }
   }
...
void SecondStage()
   ...
   for(count=1; count<steps; count++)
      ...
      BL.Wait_Until(true);   // synchronize with previous stage
      BL.SetState(false);
