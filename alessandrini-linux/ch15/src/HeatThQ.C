...
SPool TP(2);                           // two threads in pipeline pool
...
void TaskFct(void *P)
   {
   int k, count, bindex, rank;
   float t1, t2;
   int Ntot = N*M;
   std::complex<float> _Im(0.0, 1.0);

   rank = TP.GetRank();   
   if(rank==1)                // CODE FOR FIRST STAGE
      {
      t2 = 0.0;
      for(count=1; count<steps; count++)
         ...
         // Construct F1 + i * F2, insert time info. 
         // ---------------------------------------
         for(k=0; k<Ntot; ++k) 
             F1[k] += _Im * F2[k];
         ...
         fft2h(F1, M, N, -1);  // first half FFT
         Q.push(bindex);       // synchronize with next stage         
         }
      bindex = -1;
      Q.push(bindex);
      }

   if(rank==2)        // CODE FOR SECOND STAGE
      {
      Q.pop(bindex);
      while(bindex >= 0)
         {
...
int main(int argc, char **argv)
   ...
   Q.set_capacity(capacity);     // set queue capacity
