...
class Body 
    ...
    void operator()( Cell* c, tbb::parallel_do_feeder<Cell*>& feeder ) const 
        ...
        // Restore ref_count in preparation for subsequent 
        // traversal.
        // -----------------------------------------------
        c->ref_count = ArityOfOp[c->op];

        // Access a successor and decrease its reference count
        // If refcount reaches 0, launch the task that updates
        // the successor
        // ---------------------------------------------------
        for( size_t k=0; k<c->successor.size(); ++k ) 
            ...
            // Correctness checking tools might not take this into account, 
            // and report data races between different tasks, that are 
