...
SPool::~SPool()
   ...
   //std::cout << "\n SPool destructor: detaching threads " << std::endl;
   //std::cin >> ch;
   for(n=1; n<=nThreads; n++) WT[n]->detach();
   
   //std::cout << "\n SPool destructor: releasing threads " << std::endl;
   //std::cin >> ch;
   BlBarrier->ReleaseThreads();
   
   //Wait for threads to exit
   // ------------------------
   //for(n=1; n<=nThreads; n++) WT[n]->join();

   // Release memory
   // --------------
   //std::cout << "\n SPool destructor: releasing memory " << std::endl;
   //std::cin >> ch;
   for(n=1; n<=nThreads; n++) delete WT[n];
   delete [] th_id;
   delete BlBarrier;
   }
