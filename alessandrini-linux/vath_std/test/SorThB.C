...
// All rights reserved.
// **************************************
/* This is file Sor1Th.C
 *
 * (1) means Boolean Lock connector
 * ThSet environment
 *
 * THIS CODE WORKS CORRECTLY
 * 
 * Implements Successive Overrelaxation for solving a 
 * two dimensional boundary value problem. No Tchebychef
 * acceleration. Pipelined architecture to resolve data
 * dependencies.
 * 
 * Domain has NxM grid. Horizontal dimension is "Length_x" 
 * in x direction, therefore delta=Length_x/(N-1). The same
...
void ThreadFct(void *p)
   ...
   do
      ...
      #ifdef BOOST_ENV
	 {
         boost::unique_lock<boost::mutex> lock(emutex);
         anorm += error_norm;
         if(rank==1) nIter++;
         }
      #else
...
int main(int argc, char **argv)
   ...
   // ----------------------------------------------
   // Create the thread set, the pipeline connectors 
   // and the barrier
   // ----------------------------------------------

   TS = new SPool(nTh);   // main is in the gang   
   ...
   // Run the thread set
   // --------------------
   std::cout << "\n Poisson equation: pipeline with Boolean locks\n" 
             << std::endl;
